...
import { Mail, Lock, ArrowRight, Video } from 'lucide-vue-next';
...
<template>
  <div class="min-h-[calc(100vh-64px)] flex items-center justify-center px-4">
    <div class="w-full max-w-md bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 rounded-3xl p-8 shadow-2xl">
      <div class="flex flex-col items-center gap-4 mb-8">
        <div class="w-16 h-16 bg-orange-500 rounded-2xl flex items-center justify-center shadow-lg shadow-orange-500/20 rotate-12">
          <Video class="w-10 h-10 text-white -rotate-12" />
        </div>
        <h1 class="text-3xl font-bold text-zinc-100">欢迎回来</h1>
        <p class="text-zinc-400 text-sm">登录您的 V-Stream 账号以继续</p>
      </div>
      ...
      <form @submit.prevent="handleLogin" class="flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-2">
          <div class="flex justify-between items-center ml-1">
            <label class="text-xs font-bold text-zinc-500 uppercase tracking-wider">密码</label>
            <a href="#" class="text-xs text-orange-500 hover:underline">忘记密码?</a>
          </div>
          <div class="relative flex items-center bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 focus-within:border-orange-500 transition-colors">
      ...
      <div class="mt-8 pt-8 border-t border-zinc-800 text-center">
        <p class="text-zinc-400 text-sm">
          还没有账号? 
          <RouterLink to="/register" class="text-orange-500 font-bold hover:underline">立即注册</RouterLink>
        </p>
      </div>
    </div>
  </div>
